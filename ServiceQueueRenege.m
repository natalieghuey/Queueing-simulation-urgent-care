classdef ServiceQueueRenege < handle
    % ServiceQueueRenege Simulation object that keeps track of customer arrivals,
    % departures, service, and renege
    ...
    properties (SetAccess = public)
        ...
        % RenegeRate - The rate at which customers leave the queueing
        % system without being served if they wait in line too long.
        RenegeRate = 1;
    ...
    properties (SetAccess = private)
        ...
        % RenegeDist - the distribution of time until a customer reneges
        % The renege rate is random per customer.
        RenegeDist;
        ...
        % Reneged - a cell array of Customer objects. When the customer 
        % reneges, they are moved from waiting to reneged.
        Reneged = {};
    ...
    methods
        ...
        function handle_arrival(obj, arrival)
            ...
            % --------------------------------------------
            % Schedule reneging event for this customer
            renege_time = random(obj.RenegeDist);

            renege_event = Renege(obj.Time + renege_time, c);
        ...
        function handle_renege(obj, renege)
            ...
            % get ID of customer reneging
            targetId = renege.Customer.Id; 

            % Loop through every waiting customer. Extract the customer
            % from waiting. Check if the customer reneges. If they do,
            % add customer to reneged list and remove them from the
            % queue. 
